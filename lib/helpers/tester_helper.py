...
class Tester(object):
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'single':
            ckpt_name = os.path.join('outputs/'+self.cfg['exp_name'], self.cfg['checkpoint'])
        ...
        if self.cfg['mode'] == 'all':
            checkpoints_list = []
            ckpt_dir = os.path.join('outputs/' + self.cfg['exp_name'])
            for _, _, files in os.walk(ckpt_dir):
                checkpoints_list = [os.path.join(ckpt_dir, f) for f in files if f.endswith(".pth")]
