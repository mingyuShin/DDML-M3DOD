...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.cfg['max_epoch']):
            ...
            if (self.epoch % self.cfg['save_frequency']) == 0:
                os.makedirs('outputs/'+self.cfg['exp_name'], exist_ok=True)
                ckpt_name = os.path.join('outputs/'+self.cfg['exp_name'], 'checkpoint_epoch_%d' % self.epoch)
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (inputs, targets, _) in enumerate(self.train_loader):
            ...
            targets['mask_2d'] = targets['mask_2d'].bool()
            targets['mask_3d'] = targets['mask_3d'].bool()
            total_loss, stats_batch = compute_centernet3d_loss(outputs, targets, feat, self.K, self.B, self.rad, self.qi, self.obj)
