...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        # basic configuration
        self.root_dir = cfg.get('dataroot', '/home/cv3/hdd/data/KITTI3D')
        self.obj_depth = cfg['obj_depth']
        self.split = split
        self.num_classes = 3
        self.max_objs = 50
        ...
        # data split loading
        assert self.split in ['train', 'val', 'trainval', 'test', 'train1', 'val1']
        ...
        if len(self.writelist) == 1:
            self.depth_dir = os.path.join(self.data_dir, 'obj_depth')
        else:
            self.depth_dir = os.path.join(self.data_dir, 'obj_depth_all')
    ...
    def get_dep_obj(self, idx):
        img_file = os.path.join(self.depth_dir, '%06d.npy' % idx)
        assert os.path.exists(img_file)
    ...
    def __getitem__(self, item):
        ...
        # depth GT preprocessing
        if self.obj_depth:
            dep = self.get_dep_obj(index)
            if random_flip_flag == True:
                dep = dep.transpose(Image.FLIP_LEFT_RIGHT)
            dep = dep.transform(tuple(self.resolution.tolist()),
                                method=Image.AFFINE,
                                data=tuple(trans_inv.reshape(-1).tolist()),
                                resample=Image.NEAREST)
            dep = np.array(dep)
            # assert(np.max(dep) > 255)
            # # print(dep.shape)
            if random_crop_flag == True:
                targets['obj_depth'] = np.zeros_like(dep)
            else:
            # dep = dep.astype(np.float32) / 256.0
                targets['obj_depth'] = dep
